...
export async function fetchBookCover(title: string, author: string) {
    try {
        ...
        const url = `https://www.googleapis.com/books/v1/volumes?q=${query}&langRestrict=fr`
...
export async function fetchGoogleBook(id: string) {
    try {
        const url = `https://www.googleapis.com/books/v1/volumes/${id}?langRestrict=fr`
        const response = await fetch(url)
...
export async function searchBooks(query: string) {
    try {
        ...
        const url = `https://www.googleapis.com/books/v1/volumes?q=${searchQuery}&maxResults=10&langRestrict=fr`
